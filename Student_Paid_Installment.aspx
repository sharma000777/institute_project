<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/MasterPage.master" AutoEventWireup="true" CodeFile="Student_Paid_Installment.aspx.cs" Inherits="Admin_Student_Paid_Installment" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        <div class="col-xl-8 col-lg-7 col-md-7 order-0 order-md-1">
            ...
            <div class="nav-align-top">
                <ul class="nav nav-pills flex-column flex-md-row flex-wrap mb-6 row-gap-2">
                    ...
                    <li class="nav-item"><a class="nav-link active" id="Nav_PaidInstallment" href="javascript:void(0);" style="background-color:#56ca00"><i class="ri-verified-badge-line me-1_5"></i>Paid Installment</a></li>
                    ...
                    <div class="dt-action-buttons pt-3 pt-md-0" id="left_Search">
                        <div class="dt-buttons btn-group" style="width: 100%; display: flex;">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="ms-3">
                                    <p class="mb-0">My Paid Installment</p>
                                    <h5 class="mb-0"></h5>
                                </div>
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        ...
        async function renderSearchMobile() {
            try {
                const response = await Ajax_GetResult_WithoutParameter("Student_Paid_Installment.aspx", "Get_SearchMobile");
        ...
        async function renderStudent() {
            try {
                ...
                const response = await Ajax_GetResult("Student_Paid_Installment.aspx", "Get_Student", dataToSend);
                const data = JSON.parse(response.d);
                if (data.length > 0) {
                    console.log(data);
        ...
        async function renderPaidInstallment() {
            try {
                ...
                var url = "Student_Paid_Installment.aspx/Get_PaidInstallmentDetails?" + dataToSend;
                await GetPaidInstallmentDetails(url);
            } catch (err) {
                console.log(err);
            }
        }

        async function GetPaidInstallmentDetails(url) {
                    ...
                    table = $('#tblPaidInstallment').DataTable(
                        {
                            ...
                            "columns": [
                                { "data": "SlNo", "name": "SlNo", "autoWidth": true, "orderable": true },
                                { "data": "InvoiceNo", "name": "InvoiceNo", "autoWidth": true },
                                { "data": "Paid_Amount", "name": "Paid_Amount", "autoWidth": true },
        ...
        async function DateRange() {
            try {
                ...
                async function cb(start, end) {
                    ...
                    // Remove the trailing '&' character
                    dataToSend = dataToSend.slice(0, -1);
                    var url = "Student_Paid_Installment.aspx/Get_PaidInstallment_ByDate?" + dataToSend;
                    await GetPaidInstallmentDetails(url);
        ...
        async function Get_MinDate() {
            try {
                const response = await Ajax_GetResult_WithoutParameter("Student_Paid_Installment.aspx", "Get_MinDate");
                return response.d;
            } catch (err) {
                console.log(err);
            }
        ...
        async function Get_MaxDate() {
            try {
                const response = await Ajax_GetResult_WithoutParameter("Student_Paid_Installment.aspx", "Get_MaxDate");
                return response.d;
            } catch (err) {
                console.log(err);
            }
