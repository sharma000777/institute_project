<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/MasterPage.master" AutoEventWireup="true" CodeFile="Report_Due.aspx.cs" Inherits="Admin_Report_Due" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    <title>Due Report | Institute Management Software</title>
    <style>
        .table th {
            color: #433c50;
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        <div class="col-md">
            <div class="card">
                <div class="card-header row" style="justify-content: space-between;">
                    <div class="col-md-4">
                        <h5><span class="text-muted fw-light">Reports /</span>Due Report</h5>
                    </div>
                    <div class="col-md-8">
                        ...
                        <div class="d-flex">
                            ...
                            <div class="d-flex w-100 flex-wrap align-items-center justify-content-between gap-2">
                                <div class="me-2">
                                    ...
                                    <strong class="small">Dues</strong>
                                </div>
                                <div class="user-progress">
                                    <div class="d-flex justify-content-center">
                                        <sup class="mt-5 mb-0 text-heading small">₹ </sup>
                                        <h4 class="fw-medium mb-0" id="lblTotalDues"></h4>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card-body">
                    <div class="table-responsive text-nowrap">
                        <table class="table nowrap" id="tblDue">
                            <caption class="ms-4">
                                List of Due
                            </caption>
                            <thead>
    ...
    <script type="text/javascript">
        ...
        const defaultRun = async () => {
            try {
                var url = "Report_Due.aspx/Get_Due_Detail";
                await GetDue_Details(url);
                await DateRange();
                await renderCourse();
        ...
        async function DateRange() {
            try {
                ...
                async function cb(start, end, rangeName) {
                    ...
                    var url = "Report_Due.aspx/Get_Due_Detail_ByDate?" + dataToSend;
                    await GetDue_Details(url);
                    $('#tblDue').DataTable().ajax.reload();
                }
        ...
        async function Get_MinDate() {
            try {
                const response = await Ajax_GetResult_WithoutParameter("Report_Due.aspx", "Get_MinDate");
                return response.d;
            } catch (err) {
                console.log(err);
            }
        ...
        async function Get_MaxDate() {
            try {
                const response = await Ajax_GetResult_WithoutParameter("Report_Due.aspx", "Get_MaxDate");
                return response.d;
            } catch (err) {
                console.log(err);
            }
        ...
        async function GetDue_Details(url) {
            try {
                return new Promise((resolve, reject) => {

                    // Destroy existing DataTable if it exists
                    if ($.fn.DataTable.isDataTable('#tblDue')) {
                        $('#tblDue').DataTable().destroy();
                    }

                    // Initialize DataTable
                    const table = $('#tblDue').DataTable({
                        "ajax": {
                            "url": url,
                            ...
                            "dataSrc": function (json) {
                                ...
                                if (json.d && json.d.data && json.d.data.length > 0) {
                                    $("#lblTotalDues").html(json.data[0].TotalDuesAmount);
                                }
                                else {
                                    $("#lblTotalDues").html("0");
                                }
                        ...
                        "language": {
                            ...
                            "emptyTable": "No dues to display on " + selectedDateRangeName+". Please check back later or review previous records."
        ...
        async function renderCourse() {
            try {
                const courseResponse = await Ajax_GetResult_WithoutParameter("Report_Due.aspx", "Get_Course");
        ...
        async function renderSearchByStudent() {
            try {
                const response = await Ajax_GetResult_WithoutParameter("Report_Due.aspx", "Get_SearchByStudent");
        ...
        async function OnChange_CourseId() {
            try {
                ...
                var url = "Report_Due.aspx/Get_Due_Detail_ByCourse?" + dataToSend;
                await GetDue_Details(url);
        ...
        async function OnChange_SearchByMobile() {
            try {
                ...
                var url = "Report_Due.aspx/Get_Due_Detail_ByMobile?" + dataToSend;
                await GetDue_Details(url);
